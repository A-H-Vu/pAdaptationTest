...
expName = 'pAdaptionTest'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Andrew\\Documents\\Work\\Projects\\p-adaption-test\\pAdaptionTest_lastrun.py',
...
trials1 = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions('pAdaptCond1.xlsx'),
...
for thisTrials1 in trials1:
    ...
    trial1Num.text = str(trials1.thisN+1)+' / 28'
...
trials2 = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('pAdaptCond2.xlsx'),
...
for thisTrials2 in trials2:
    ...
    #print('trial: '+str(trials1.thisN)+' ('+str(globalClock.getTime())+')')
    trial2Num.text = str(trials2.thisN+1)+' / 80'
    ...
    _trial2Skip_allKeys = []
    # keep track of which components have finished
    trial2Components = [trial2Mouse, trial2Target, trial2Home, trial2Cursor, trial2Num, trial2Skip]
    ...
    while continueRoutine:
        ...
        if trial2Cursor.status == STARTED:  # only update if drawing
            trial2Cursor.setPos(((trial2Mouse.getPos()[0]*cos(r))-(trial2Mouse.getPos()[1]*sin(r)), (trial2Mouse.getPos()[0]*sin(r))+(trial2Mouse.getPos()[1]*cos(r))), log=False)
...
trials3 = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('pAdaptCond3.xlsx'),
...
for thisTrials3 in trials3:
    ...
    _trial3Skip_allKeys = []
    # keep track of which components have finished
    trial3Components = [trial3Mouse, trial3Target, trial3Home, trial3Cursor, trial3Num, trial3Skip]
    ...
    while continueRoutine:
        ...
        if trial3Cursor.status == STARTED:  # only update if drawing
            trial3Cursor.setPos(((trial2Mouse.getPos()[0]*cos(k))-(trial2Mouse.getPos()[1]*sin(k)), (trial2Mouse.getPos()[0]*sin(k))+(trial2Mouse.getPos()[1]*cos(k))), log=False)
